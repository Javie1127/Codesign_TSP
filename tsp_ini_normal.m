function [fdcomm, radar, cov] = tsp_ini_normal(radar,fdcomm,radar_comm)
%%%%-----------------------------------
...
%     d_ui    = d_UL(ii);
%     N_ui    = N_UL(ii);
%     Sigma_ui = [P_UL_i/N_ui*eye(d_ui);zeros(N_ui-d_ui,d_ui)];
...
% for jj = 1:J
%     d_dj = d_DL(jj);
%     Sigma_dj = [P_DL/J*eye(d_dj);zeros(Mc-d_dj,d_dj)];
