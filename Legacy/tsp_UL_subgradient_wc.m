...
while t <= tu_max
    ...
    R_ui_k_t = real(log2(det(nearestSPD(eye(d_UL_i)+nearestSPD(P_ui_k_t'*HiB'/R_in_ui*HiB*P_ui_k_t)))));
    ...
    cov_temp = tsp_covmat_wc(fdcomm_temp, radar_temp, radar_comm);
...
%     Xi_temp = fdcomm_temp.Xi_UL(k)+fdcomm_temp.Xi_DL(k) + sum(radar_temp.Xi_r);
    ...
    R_ui_k_temp = real(log2(det(eye(d_UL_i)+nearestSPD(P_ui_k_temp'*HiB'/R_in_ui_temp*HiB*P_ui_k_temp))));
...
cov = tsp_covmat_wc(fdcomm, radar, radar_comm);
%% Update MSE matrix
