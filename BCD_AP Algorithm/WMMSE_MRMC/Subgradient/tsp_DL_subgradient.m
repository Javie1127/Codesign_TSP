...
while t <= td_max
    ...
    P_dj_k = fdcomm_temp.DLprecoders{jj,k};
    P_B_t = P_dg_k_sum + abs(trace((P_dj_k*P_dj_k')));
    beta_d_k_t = 1/(t);
    %beta_d_k_t = 1/t/norm((P_B_t-P_B_max));
...
%     R_jd_k_t = log2(det((E_jd_k_t)^(-1)));
    ...
    %% update P_jd_k
    fdcomm_temp = tsp_DL_precoders(k, jj, fdcomm_temp, cov_temp);
...
%     Xi_t = fdcomm_temp.Xi_UL(k)+ fdcomm_temp.Xi_DL(k) + sum(radar_temp.Xi_r);
    ...
    R_dj_k_t_temp = abs(log2(det(eye(d_DL_j)+(P_dj_k_temp'*HBj'/R_in_dj_temp*HBj*P_dj_k_temp))));
