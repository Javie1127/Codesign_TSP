function [fdcomm, radar, cov] = tsp_UL_subgradient(fdcomm, radar_comm, radar, cov, ii,k)
...
while t <= tu_max
    ...
    cov_temp = tsp_covmat_rev(fdcomm_temp, radar_temp, radar_comm,cov_temp);
    P_ui_k_temp = fdcomm_temp.ULprecoders{ii,k};
    R_UL_i_k = cov_temp.total_UL{ii,k};
    fdcomm_temp.UL_MMSE_nop{ii,k} = ...
        eye(fdcomm_temp.ULstream_num(ii))-2*U_ui_k*HiB*P_ui_k_temp + (U_ui_k*R_UL_i_k*U_ui_k');
    %% update Xi_MSE
    fdcomm_temp = Xi_comm_k(fdcomm_temp, k);
%     radar_temp = Xi_radar(radar_temp);
