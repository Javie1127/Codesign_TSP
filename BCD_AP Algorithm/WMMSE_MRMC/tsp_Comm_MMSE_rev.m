...
for k = 1:K
    for jj = 1 : J
        ...
        d_new = eye(size(E_dj_k_star), 'logical');
        E_dj_k_star(d_new) = real(diag(E_dj_k_star));
...
%         xi_DL{k,jj} = alpha_dj*(U_dj_k'*W_dj_k*U_dj_k);
        xi_DL{k,jj} = alpha_dj*real(U_dj_k'*(E_dj_k_star\U_dj_k));
...
for k = 1:K
    ...
    for ii = 1 : I
        ...
        d_new = eye(size(E_ui_k_star), 'logical');
        E_ui_k_star(d_new) = real(diag(E_ui_k_star));
        ...
        %E_UL{ii,k} = E_ui_k;
        xi_UL_k = xi_UL_k + alpha_ui*real(U_ui_k'*(E_ui_k_star\U_ui_k));
    end
    xi_UL(:,:,k) = xi_UL_k;
end
