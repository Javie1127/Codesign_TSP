...
radar.codelength = K;
%% FD Comm parameters
Mc = 0; % Number of BS TX antennas
Nc = 4;% Number of BS RX antennas
I = 2; % Number of UL UEs
J = 0; % Number of DL UEs
fdcomm.BSTX = Mc;
fdcomm.BSRX = Nc;
fdcomm.UL_num = I;
fdcomm.DL_num = J;
...
while ell <= ell_max
    ...
    radar_uew_ni = tsp_radar_MMSE(radar, cov_uew_ni);
    ...
    if I_total_uew_ni(ell) > I_max_uew_ni
        I_total_op_uew_ni(ell) = I_total_m_uew_ni(ell);
        I_max_uew_ni = I_total_m_uew_ni(ell);
    else
