...
%% Parameters to save
CNR_sweep = -30:5:30;
radar_save = cell(length(JJ),1);
fdcomm_save = cell(length(JJ),1);
cov_save = cell(length(JJ),1);
%% Loop through CNRs 
...
for ii = 1:length(CNR_sweep)
    ...
    [fdcomm_op, radar_op,cov_op, ~] = tsp_altermating_projection_v1(fdcomm_ini, radar_ini,radar_comm,cov_temp);
