function tsp_codesign_runCode_multi_case_test_RI(configureXML)
...
for ii = 1:length(iter_variables)
    ...
    for d=1:num_iter % num_iter channel realizations
        ...
        fdcomm_rept = cell(num_rept,1);
        radar_rept = cell(num_rept,1);
        I_rept = zeros(num_rept,1);
        parfor q = 1:num_rept % num_rep > 1 for the random initialization only otherwise num_rep = 1
            disp(d);
...
%             end
            %% BCD-AP algorithm 
            [fdcomm_tmp_q, radar_tmp_q] = tsp_BCD_AP(fdcomm, radar, radar_comm_d, cov);
            fdcomm_rept{q} = fdcomm_tmp_q;
            radar_rept{q} = radar_tmp_q;
            I_rept(q) = max(fdcomm_tmp_q.I_total_op);
        end
        %% Update Stats
        %stats_temp = collect_stats(fdcomm_op_d,radar_op_d,stats_ini);
        [~,q_max] = max(I_rept);
        fdcomm_op_d = fdcomm_rept{q_max};
        radar_op_d = radar_rept{q_max};
...
saveSimulationData(systemCfg,stats_total, fdcomm_op_d,radar_op_d, configureXML,'final');
